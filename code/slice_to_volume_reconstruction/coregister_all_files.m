...
for i = 1:length(dir_list)
    ...
    file_list = dir(fullfile(dir_path, '*downsampled*'));
    ...
    for j = 1:2:length(file_list)
        ...
        filename1 = fullfile(dir_path, file_list(j).name);
        filename2 = fullfile(dir_path, file_list(j+1).name);
        
        % Call the coregister_to_epi function
        coregister_to_epi(filename1, filename2);
