...
def main(args):
    ...
    for directory in os.listdir(args.data_path):
        ...
        mirtk_command = f"nice -n 10 mirtk reconstruct output.nii.gz 2 {files[1]} {files[0]} -resolution {args.resolution} -template {files[1]} -no_registration -iterations {args.iterations}"
        ...
        for file in os.listdir(os.path.join(args.data_path, directory)):
            if file not in [f"output.nii.gz", files[0], files[1], files[2]]:
